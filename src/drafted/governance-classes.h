 ...
 *
 *   // network
 *   CGermanCCNetwork lvl, network-type, network-status, network-error, milestone-status*
...
private:
    ...
    // Current OBJECT STATUS (see http://govman.germancc.org/index.php/Documentation_:_Status_Field)
    int nStatusID;
...
// // root node
class CGermanCCNetwork : public CGovernanceObject
{
private:
    std::string strName;
    std::string strURL;
...
public:
    CGermanCCNetwork(UniValue objIn)
    {
...
// // can be under: GermanCCNetwork
// //   -- signature requirements : Key1(User)
// class CGermanCCNetworkVariable : public CGovernanceObject
// {
// private:
