...
const char * const BITCOIN_CONF_FILENAME = "germancc.conf";
const char * const BITCOIN_PID_FILENAME = "germanccd.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "germancc" is a composite category enabling all GermanCC-related debug output
            if(ptrCategory->count(string("germancc"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\GermanCCCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\GermanCCCore
    // Mac: ~/Library/Application Support/GermanCCCore
    // Unix: ~/.germancccore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "GermanccCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/GermanccCore";
#else
    // Unix
    return pathRet / ".GermanccCore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty germancc.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override germancc.conf
